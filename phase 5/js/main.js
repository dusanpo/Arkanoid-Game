...
document.addEventListener("keydown", function (event) {
    ...
    if (taster === 32 && laserPostoji && !laserAktiviran) {
        laser.style.top = palica.offsetTop - 30 + 'px';
        laser.style.left = palica.offsetLeft + 90 + 'px';
        laserAktiviran = true
...
function pravimoCigle(red) {
    ...
    //div.innerHTML = racunanjePoena;
    if (racunanjePoena > 6 && racunanjePoena < 10) {
        div.setAttribute("tip", LASER);
...
function info() {
    if (krajIgre === false) {
        ...
        proveraNagrada();
        if (laserPostoji && !palica.style.backgroundImage) {
            palica.style.backgroundImage = "url('img/palica.png')";
            palica.style.borderLeft = 0;
            palica.style.borderRight = 0;
            palica.style.width = '180px';
        }

        if (!laserPostoji && palica.style.backgroundImage) {
            palica.style.backgroundImage = '';
            palica.style.borderLeft = '15px solid  #b1ff33';
            palica.style.borderRight = '15px solid  #b1ff33';
            palica.style.width = '150px';
        }
        kretanjeLasera();
        animacija = requestAnimationFrame(info);
    }
...
function igraJeZavršena() {
    document.getElementById("pocetak").style.display = "block";
    krajIgre = true;
